...
`zizu` - монорепозиторий проекта по РБПО. В корне лежит серверная часть на Spring Boot, а в `windows-client/` - отдельная клиентская часть для Windows.
...
- `windows-client/`: клиентская часть на `C++` и `WinUI 3`
